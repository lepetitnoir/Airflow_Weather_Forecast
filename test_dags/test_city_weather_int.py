...
def compute_model_score_lr(model, X, y):
    # computing cross val
...
def linreg_score(task_instance):
    ...
    X, y = prepare_data_lr()
    lr_score = compute_model_score_lr(model, X, y)
...
def compute_model_score_dt(model, X, y):
    # computing cross val
    cross_validation = cross_val_score(
        model,
        X,
    ...
    score_dt = cross_validation.mean()
    return score_dt

def dtree_score(task_instance):
    model = DecisionTreeRegressor()
    X, y = prepare_data_dt()
    dt_score = compute_model_score_dt(model, X, y)
    task_instance.xcom_push(
        key="dt_model_score",
        value={
        dt_score
...
def compute_model_score_rf(model, X, y):
    # computing cross val
    cross_validation = cross_val_score(
        model,
        X,
...
def randfor_score(task_instance):
    ...
    X, y = prepare_data_rf()
    rf_score = compute_model_score_rf(model, X, y)
    task_instance.xcom_push(
        key="rf_model_score",
        value={
        rf_score
